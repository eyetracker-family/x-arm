...
int grasp_type;// 0:ball, 1:bottle
...
void BoundingBox_callback(const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg) 
{
	...
	for(int i=0;i<num;i++)
	{
        if(grasp_type==0)
        {
		    if(msg->bounding_boxes[i].Class=="sports ball")//bottle
		    {
        ...
        if(grasp_type==1)
        {
		    if(msg->bounding_boxes[i].Class=="bottle")//bottle
		    {
			    detected_object temp;
			    temp.classname=msg->bounding_boxes[i].Class;
			    temp.probability=msg->bounding_boxes[i].probability;
...
pcl::PointCloud<pcl::PointXYZ>::Ptr MatToPoinXYZ(cv::Mat ff)
{
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ptr(new pcl::PointCloud<pcl::PointXYZ>);
	for(int y=0;y<disp.rows;y++)
		for(int x=0;x<disp.cols;x++)
		{
			if(xyz.at<Vec3f>(y,x)[2]<8000.0)
			{
				pcl::PointXYZ point;
				point.x = xyz.at<Vec3f>(y,x)[0];
				point.y = xyz.at<Vec3f>(y,x)[1];
				point.z = xyz.at<Vec3f>(y,x)[2];
